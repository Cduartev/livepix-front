...
import { usePixStore, type PixCharge, type PixStatus } from "@/stores/usePixStore";
...
export default function QrOverlayPage() {
  ...
  // store
  const enqueue = usePixStore((s) => s.enqueue);
  const closeActive = usePixStore((s) => s.closeActive);
  const setActive = usePixStore((s) => s.setActive);
  const activePaymentId = usePixStore((s) => s.activePaymentId);
  const active = usePixStore((s) => s.getActive());
  const queue = usePixStore((s) => s.queue);
  ...
  async function gerarPix() {
    ...
    try {
      ...
      const normalizedStatus: PixStatus = data.status
        ? usePixStore.getState().normalizeStatus(data.status, undefined)
        : "PENDING";
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (diff <= 0) {
        setCountdown("00:00");
        usePixStore.getState().updateStatus(active.paymentId, "EXPIRED");
        return;
      }
